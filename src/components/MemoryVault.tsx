    ...
    const handleAddMemory = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (newMemory.type === 'photo' && previewImages.length > 0) {
                newMemories = [{ id: Date.now(), location: newMemory.location || "Unknown Location", date, content: newMemory.content, type: "photo", images: previewImages }];
            } else {
                newMemories = [{ id: Date.now(), location: newMemory.location || "Unknown Location", date, content: newMemory.content, type: newMemory.type }];
            }
        ...
        <div className="w-full max-w-6xl space-y-12 px-4 md:px-8 pb-32">
            ...
            <div className="flex flex-col space-y-12 items-center w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
                    <AnimatePresence mode="popLayout">
                        {memories.map((memory, index) => {
                            ...
                            return (
                                <motion.div key={memory.id} initial={{ opacity: 0, y: 50, rotate: 5 }} animate={{ opacity: 1, y: 0, rotate: 0 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ type: "spring", stiffness: 200, damping: 20, delay: index * 0.1 }} layout className="group cursor-pointer relative">
                                    ...
                                    <div className={`pop-card p-6 rounded-2xl h-full flex flex-col space-y-4 ${rotateClass} hover:rotate-0 hover:scale-105 transition-transform duration-300 relative overflow-hidden`}>
                                        ...
                                        {memory.type === 'photo' && (
                                            <div className="w-full bg-white p-1 pb-4 border-2 border-pop-espresso shadow-[2px_2px_0px_0px_rgba(0,0,0,0.1)] transform rotate-1 relative z-10 group-hover:rotate-0 transition-transform">
                                                {memory.images && memory.images.length > 0 ? (
                                                    <div className={cn("grid gap-1", memory.images.length === 1 ? "grid-cols-1" : "grid-cols-2")}>
                                                        {memory.images.map((img, i) => (
                                                            <div key={i} className={cn("relative aspect-square overflow-hidden bg-gray-100 border border-black/10", memory.images?.length === 3 && i === 0 ? "col-span-2 aspect-video" : "")}>
                                                    ...
                                                    <div className="aspect-[4/3] flex items-center justify-center text-pop-espresso/30 flex-col space-y-2 bg-gray-100"><Camera className="w-8 h-8" /></div>
                                                )}
                                            </div>
                                        )}
                                        <div className="flex-grow relative z-10 pt-2">
                                            <p className="font-bold text-pop-espresso leading-relaxed text-lg" style={{ fontFamily: '"Comic Neue", "cursive", sans-serif' }}>"{memory.content}"</p>
                                        </div>
                                        <div className="pt-4 flex items-center justify-between border-t-2 border-pop-espresso/10 border-dashed relative z-10">
                                            ...
                                            <button className="text-pop-espresso hover:text-pop-pink transition-colors"><Share2 className="w-5 h-5" /></button>
                                        </div>
                                    </div>
                                </motion.div>
                            );
                ...
                <AnimatePresence>
                    {isAdding && (
                        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-sepia-900/60 backdrop-blur-md">
                            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="w-full max-w-md bg-white rounded-2xl md:rounded-[40px] p-5 md:p-8 relative shadow-2xl overflow-y-auto max-h-[90vh] border-4 border-pop-espresso">
                                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-sepia-300 via-accent-gold to-sepia-300" />
                                <button onClick={() => setIsAdding(false)} className="absolute top-4 right-4 md:top-6 md:right-6 p-2 rounded-full hover:bg-sepia-50 text-sepia-300 transition-colors"><X className="w-5 h-5" /></button>
                                <form onSubmit={handleAddMemory} className="space-y-6">
                                    <div className="text-center space-y-2 pt-4">
                                        <div className="w-12 h-12 rounded-2xl bg-pop-yellow flex items-center justify-center mx-auto mb-4 border-2 border-pop-espresso shadow-[2px_2px_0px_0px_var(--color-pop-espresso)]"><Sparkles className="w-6 h-6 text-pop-espresso" /></div>
                                        <h3 className="text-2xl font-black text-pop-espresso uppercase tracking-tight">Capture a Moment</h3>
                                        <div className="inline-block bg-pop-green px-3 py-1 rounded-full border-2 border-pop-espresso text-[10px] font-black uppercase tracking-widest text-pop-espresso">Tag #{tagId}</div>
                                    </div>
                                    <div className="space-y-4">
                                        <div className="flex p-1 bg-sepia-50 rounded-2xl border border-sepia-100">
                                            <button type="button" onClick={() => setNewMemory({ ...newMemory, type: 'note' })} className={cn("flex-1 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all", newMemory.type === 'note' ? "bg-white text-sepia-900 shadow-sm" : "text-sepia-300")}>Quick Note</button>
                                            <button type="button" onClick={() => setNewMemory({ ...newMemory, type: 'photo' })} className={cn("flex-1 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all", newMemory.type === 'photo' ? "bg-white text-sepia-900 shadow-sm" : "text-sepia-300")}>Photo & Text</button>
                                        </div>
                                                            ...
                                                            <div className="grid grid-cols-2 gap-1 w-full h-full p-2">
                                                                {previewImages.slice(0, 4).map((img, i) => (
                                                                    <div key={i} className="relative aspect-square">
                                                                        ...
                                                                        {i === 3 && previewImages.length > 4 && (<div className="absolute inset-0 bg-black/50 rounded-lg flex items-center justify-center text-white font-bold">+{previewImages.length - 4}</div>)}
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        ) : (
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input required value={newMemory.location} onChange={(e) => setNewMemory({ ...newMemory, location: e.target.value })} placeholder="Piazza Navona..." className="w-full bg-sepia-50/30 border border-sepia-100 rounded-2xl py-3 pl-10 pr-4 outline-none focus:border-accent-gold transition-colors text-sm" />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <div className="flex items-center justify-between ml-1">
                                                <label className="text-[10px] uppercase tracking-widest font-black text-sepia-300">The Memory</label>
                                                <span className="text-[8px] font-bold text-accent-gold uppercase tracking-tighter animate-pulse">Prompt: {(() => {
                                                    const prompts = ["What does this moment sound like?", "If this memory had a scent, what would it be?", "Who are you with and what are they laughing at?", "What's the one thing you never want to forget about this spot?", "Write a message to your future self about today."];
                                                    return prompts[new Date().getDate() % prompts.length];
                                                })()}</span>
                                            </div>
                                            <textarea required value={newMemory.content} onChange={(e) => setNewMemory({ ...newMemory, content: e.target.value })} placeholder="Write your feelings here..." className="w-full bg-sepia-50/30 border border-sepia-100 rounded-2xl py-4 px-4 h-32 outline-none focus:border-accent-gold transition-colors text-sm resize-none serif-font italic text-lg" />
                                        </div>
                                    </div>
                                    <button type="submit" disabled={isSaving} className="w-full py-4 rounded-xl bg-pop-yellow text-pop-espresso font-black uppercase tracking-widest border-4 border-pop-espresso shadow-[4px_4px_0px_0px_var(--color-pop-espresso)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_var(--color-pop-espresso)] active:translate-x-[4px] active:translate-y-[4px] active:shadow-none transition-all flex items-center justify-center space-x-2">
                                        {isSaving ? (<><Loader2 className="w-5 h-5 animate-spin" /><span>Saving...</span></>) : (<span>Save to My Vault</span>)}
                                    </button>
                                </form>
                            </motion.div>
                        </div>
                ...
                <div className="text-center pt-12 pb-20">
                    <p className="text-sepia-300 text-[10px] font-black tracking-[0.4em] uppercase">Souvenir ID: RM-772-BX-2026</p>
                </div>
            </div>
        </div>
    );
